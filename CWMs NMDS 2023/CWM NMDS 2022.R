...
CWM.Sites22 <-
  as.data.frame(scores(CWM.NMDS.2022, "sites")) # Get the site coordinates
CWM.Sites22$Lake <- Casc22_Site_Meta$Lake #add context
CWM.Sites22$Year <- Casc22_Site_Meta$Year #add context
CWM.Sites22$Fish.Status <- Casc22_Site_Meta$Fish.Status #add context
write.csv(CWM.Sites22, "CWM.Sites22.csv") #Write it as a excel table so you have a "hardcopy"




CWM.Traits22 <- as.data.frame(scores(CWM.NMDS.2022, "species"))  # Get the spp coordinates

row.names(CWM.Traits22) <- c(
  "Carnivore",
  "Herbivore",
  "Omnivore",
  "Filter Feeding",
...
CWM.22 <- ggplot() +
  ...
  geom_point(
    data = CWM.Sites22,#Makes circles at year points
    aes(x = NMDS1,
        y = NMDS2),
    col = "black",
    pch = 21,
  ...
  geom_text_repel(data = CWM.Traits22, #Place Trait names
    aes(
      x = NMDS1,
      y = NMDS2,
...
# x-range
global.x.lims <- layer_scales(CWM.22)$x$range$range

#y-range
global.y.lims <- layer_scales(CWM.22)$y$range$range
